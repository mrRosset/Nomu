...
void E32ImageLoader::load(E32Image& image, std::string& file_name, MemoryInterface& mem, std::string& lib_folder_path) {
...
void E32ImageLoader::parse(std::string& path, E32Image& image)
{
	LoadFile(path, image.data);

	parseHeader(image);
	checkHeaderValidity(image);

	if (!image.valid_uid_checksum) {
		throw std::invalid_argument("Wrong uid checksum, not a valid E32Image.");
	}
	if (!image.valid_signature) {
		throw std::invalid_argument("Not an E32Image.");
	}
	if (image.header->flags.header_format > 1) {
		throw std::invalid_argument("Not a basic header or J-header. V-headers are not supported");
	}
	if (image.header->flags.import_format != 0 && image.header->flags.import_format != 2) {
		throw std::invalid_argument("ELF import section not supported");
	}
	...
	if (!image.valid_imports) {
		throw std::runtime_error("Incoherence between the Import Address Table and the Import section");
	}
