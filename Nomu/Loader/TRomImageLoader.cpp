...
void TRomImageLoader::load(TRomImage& image, MemoryInterface& mem, std::string& lib_folder_path) {
	//What is there to do here ?
}

void TRomImageLoader::parse(std::string& path, TRomImage& image) {
	...
	if (!image.valid_uid_checksum) {
		throw std::invalid_argument("Wrong uid checksum, not a valid TRomImage.");
	}
}
