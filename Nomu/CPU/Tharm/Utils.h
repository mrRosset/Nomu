...
inline void u32_to_PSR(u32 input, PSR& psr) {
	...
	psr.flag_T = getFlagBit(input, 5);
	psr.flag_inter_F = getFlagBit(input, 6);
	psr.flag_inter_I = getFlagBit(input, 7);
	psr.reserved = (input >> 8) & 0xFFFFF;
	psr.flag_V = getFlagBit(input, 28);
	psr.flag_C = getFlagBit(input, 29);
	psr.flag_Z = getFlagBit(input, 30);
	psr.flag_N = getFlagBit(input, 31);
}